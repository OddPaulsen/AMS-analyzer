...
/**
 * Created by paulseno on 13.09.2017, 22:46.
 * Taken from https://github.com/Gurux
 * Slightly modified to use List<> instead of []
 */
public class GXFCS16 {
     ...
     * @return
     */
    public static final int countFCS16(List<Integer> buff, final int offset, final int count) {
        int fcs16 = 0xFFFF;
        for (int pos = offset; pos < offset + count; ++pos) {
            fcs16 = (((fcs16 >> 8) ^ fcs16Table[(fcs16 ^ buff.get(pos)) & 0xFF]) & 0xFFFF);
        }
        fcs16 = ~fcs16;
